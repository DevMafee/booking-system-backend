import {
  ...
  ArgumentMetadata,
  BadRequestException,
  ValidationPipeOptions, Optional,
} from '@nestjs/common';
import { validate } from 'class-validator';
import { plainToClass } from 'class-transformer';
import { ParamValidationException } from '../exceptions/paramValidationException';
import { IValidationError, ValidationException } from '../exceptions/validationException';
  ...
  async transform(value: any, { metatype, type }: ArgumentMetadata) {
    try {
      ...
      if (errors.length > 0) {
        ...
        if (type === 'param') {
          throw new ParamValidationException(validationErrors.map(error => error.message));
        } else {
