...
export class FileUploadService {
  ...
  IsExistsTempFile = (fileName: string)=>{
    try {
      if(fileName.length>2){
        ...
        if (!fs.existsSync(`${process.env.TEMP_FILES_DIR}/${fileName}`)) return false
        return true;
      }
      return false;
    }catch (err) {
  ...
  tempFileValidationCheckIfDataExists = (fileName: string, fieldName: string)=>{
    if(!fileName){
      return;
    }
    const isTempFileExists = this.IsExistsTempFile(fileName);
  ...
  fileValidationCheckAndMove = (fileName: string, fieldName: string)=>{
    const isTempFileExists = this.IsExistsTempFile(fileName);
  ...
  fileValidationCheckIfDataExists = (fileName: string, fieldName: string, message?: string)=>{
    if(!fileName){
      return;
    }
    const isTempFileExists = this.IsExistsTempFile(fileName);
