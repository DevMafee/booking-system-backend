...
@Controller('v1/admin/admin-users')
@ApiTags('Admin Users')
@UseGuards(AuthGuard)
@ApiBearerAuth('JWT')
...
export class UsersController {
  ...
  async findAll(
    ...
    const [users, total] = await this.adminUsersService.findAll(filter, pagination, adminUser);
  ...
  async findAllList() {
    const [user, c] = await this.adminUsersService.findAllList();
    return new PayloadResponseDTO({
      statusCode: HttpStatus.OK,
  ...
  // Update user Status
  @Patch(':id/status')
  @ApiResponse({ description: 'Single User Status Changed', status: HttpStatus.OK})
  async status(
    @AdminUser() adminUser: AdminUserDto,
    @Param(new DtoValidationPipe()) params :AdminUserIdParamDto,
    @Body(new DtoValidationPipe()) statusChangeAdminUserDto: StatusChangeAdminUserDto,
  ) {
